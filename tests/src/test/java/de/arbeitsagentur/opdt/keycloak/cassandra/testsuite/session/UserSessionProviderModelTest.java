...
public class UserSessionProviderModelTest extends KeycloakModelTest {
    ...
    public void testUpdateSession() {
        ...
        withRealm(realmId, (s, r) -> {
            ...
            assertEquals(lastRefresh, s.sessions().getUserSession(r, sessions[0].getId()).getLastSessionRefresh());
            return null;
        });
    }
    ...
    public void testRestartSession() {
        ...
        try {
            withRealm(realmId, (s, r) -> {
                ...
                assertSession(userSession, s.users().getUserByUsername(r, "user1"), "127.0.0.1", started, started, "test-app", "third-party");

                userSession.restartSession(r, s.users().getUserByUsername(r, "user2"), "user2", "127.0.0.6", "form", true, null, null);

                userSession = s.sessions().getUserSession(r, sessions[0].getId());
                assertSession(userSession, s.users().getUserByUsername(r, "user2"), "127.0.0.6", started + 100, started + 100);
                return null;
            });
        } finally {
            Time.setOffset(0);
    ...
    public void testCreateClientSession() {
        withRealm(realmId, (s, r) -> {
            UserSessionModel[] sessions = createSessions(s, r.getId());
            Map<String, AuthenticatedClientSessionModel> clientSessions = s.sessions().getUserSession(r, sessions[0].getId()).getAuthenticatedClientSessions();
            ...
            assertEquals(r.getClientByClientId("test-app").getClientId(), session1.getClient().getClientId());
    ...
    public void testUpdateClientSession() {
        withRealm(realmId, (s, r) -> {
            ...
            AuthenticatedClientSessionModel updated = s.sessions().getUserSession(r, userSessionId).getAuthenticatedClientSessions().get(clientUUID);
    ...
    public void testUpdateClientSessionWithGetByClientId() {
        withRealm(realmId, (s, r) -> {
            ...
            AuthenticatedClientSessionModel updated = s.sessions().getUserSession(r, userSessionId).getAuthenticatedClientSessionByClient(clientUUID);
            assertEquals(AuthenticatedClientSessionModel.Action.LOGGED_OUT.name(), updated.getAction());
            assertEquals(time + 10, updated.getTimestamp());
            return null;
        });
    ...
    public void testGetUserSessions() {
        withRealm(realmId, (s, r) -> {
            UserSessionModel[] sessions = createSessions(s, r.getId());
            assertSessions(s.sessions().getUserSessionsStream(r, s.users().getUserByUsername(r, "user1"))
                .collect(Collectors.toList()), sessions[0], sessions[1]);
            assertSessions(s.sessions().getUserSessionsStream(r, s.users().getUserByUsername(r, "user2"))
    ...
    public void testRemoveUserSessionsByUser() {
        ...
        withRealm(realmId, (s, r) -> {
            clientSessionsKept.putAll(s.sessions().getUserSessionsStream(r,
                    s.users().getUserByUsername(r, "user2"))
                .collect(Collectors.toMap(UserSessionModel::getId, model -> model.getAuthenticatedClientSessions().keySet().size())));
        ...
        withRealm(realmId, (s, r) -> {
            assertEquals(0, s.sessions().getUserSessionsStream(r, s.users().getUserByUsername(r, "user1"))
                .count());
            List<UserSessionModel> userSessions = s.sessions().getUserSessionsStream(r,
                s.users().getUserByUsername(r, "user2")).collect(Collectors.toList());
    ...
    public void testRemoveUserSessionsByRealm() {
        withRealm(realmId, (s, r) -> {
            ...
            assertEquals(0, s.sessions().getUserSessionsStream(r, s.users().getUserByUsername(r, "user1"))
                .count());
            assertEquals(0, s.sessions().getUserSessionsStream(r, s.users().getUserByUsername(r, "user2"))
                .count());
            return null;
        });
    }
    ...
    public void testOnClientRemoved() {
        withRealm(realmId, (s, r) -> {
            ...
            for (UserSessionModel session : sessions) {
                ...
                Set<String> clientUUIDS = new HashSet<>(session.getAuthenticatedClientSessions().keySet());
    ...
    public void testTransientUserSession() {
        ...
        withRealm(realmId, (s, r) -> {
            ...
            UserSessionModel userSession = s.sessions().createUserSession(userSessionId, r, s.users().getUserByUsername(r, "user1"),
            ...
            assertSession(userSession, s.users().getUserByUsername(r, "user1"), "127.0.0.1", userSession.getStarted(), userSession.getStarted(), "test-app");
    ...
    public void testGetByClient() {
        withRealm(realmId, (s, r) -> {
            ...
            KeycloakModelUtils.runJobInTransaction(s.getKeycloakSessionFactory(), (KeycloakSession kcSession) -> {
                assertSessions(kcSession.sessions().getUserSessionsStream(r, r.getClientByClientId("test-app"))
                    .collect(Collectors.toList()), sessions[0], sessions[1], sessions[2]);
                assertSessions(kcSession.sessions().getUserSessionsStream(r, r.getClientByClientId("third-party"))
    ...
    public void testGetByClientPaginated() {
        withRealm(realmId, (s, r) -> {
            ...
            try {
                for (int i = 0; i < 25; i++) {
                    Time.setOffset(i);
                    UserSessionModel userSession = s.sessions().createUserSession(realm, s.users().getUserByUsername(realm, "user1"), "user1", "127.0.0." + i, "form", false, null, null);
                    AuthenticatedClientSessionModel clientSession = s.sessions().createClientSession(realm, realm.getClientByClientId("test-app"), userSession);
    ...
    public void testCreateAndGetInSameTransaction() {
        withRealm(realmId, (s, r) -> {
            ...
            AuthenticatedClientSessionModel clientSession = createClientSession(s, realmId, client, userSession, "http://redirect", "state");
